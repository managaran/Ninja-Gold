...
def process_money():
    ...
    if request.form['building'] == 'farm':
        ...
        session['activities'].append(
            f"<div class='won'>Earned {number} gold from the farm! ({time})</div>")
        ...
        print(f"Total Gold = {session['total_gold']}")
        session['activities'].append(
            f"<div class='won'>Earned {number} gold from the cave! ({time})</div>")
        # print(session['activities'])
        ...
        print(f"Total Gold = {session['total_gold']}")
        session['activities'].append(
            f"<div class='won'>Earned {number} gold from the house! ({time})</div>")
        # print(session['activities'])
        ...
        if session['total_gold'] > 0:
            ...
            if number > 0:
                print(f"Gold increased by {number}.")
                session['activities'].append(
                    f"<div class='won'>Earned {number} gold from the casino! ({time})</div>")
            if number < 0:
            ...
            session['message'] = ''
            session['activities'].append(
                f"<div>You have no gold to gamble. Please come back to the casino when you have more money.</div>")
            print(f"Total Gold = {session['total_gold']}")
            print(
                "You have no gold to gamble. Please come back to the casino when you have more money.")
